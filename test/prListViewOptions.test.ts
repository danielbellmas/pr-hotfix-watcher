...
describe("normalizePrListViewOptions", () => {
  ...
  it("accepts valid partials", () => {
    expect(
      normalizePrListViewOptions({ statusFilter: "open", sortMode: "created" })
    ).toEqual({
      statusFilter: "open",
      sortMode: "created",
    });
  ...
  it("preserves explicit all and status when defaults differ", () => {
    ...
    expect(
      normalizePrListViewOptions({ statusFilter: "all" }, altDefaults)
    ).toEqual({
      statusFilter: "all",
      sortMode: "created",
    });
    expect(
      normalizePrListViewOptions({ sortMode: "status" }, altDefaults)
    ).toEqual({
      statusFilter: "open",
      sortMode: "status",
    });
...
describe("applyPrViewFilterSort", () => {
  ...
  it("sort status: open before merged, then newest created", () => {
    const out = applyPrViewFilterSort(
      [r3, r1, r4, r2],
      "all",
      "status",
      new Set()
  ...
  it("tie-breaks created sort by PR number descending", () => {
    ...
    expect(
      applyPrViewFilterSort([a, b], "all", "created", new Set()).map(
        (r) => r.number
      )
    ).toEqual([2, 1]);
  });
  ...
  it("invalid createdAt sorts as 0 but tie-break still orders", () => {
    ...
    expect(
      applyPrViewFilterSort([bad, good], "all", "created", new Set()).map(
        (r) => r.number
      )
    ).toEqual([6, 5]);
  });
